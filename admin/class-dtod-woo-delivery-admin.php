 ...
 * @subpackage dtod_Woo_Delivery/admin
 */
...
class dtod_Woo_Delivery_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( "flatpickr_css", dtod_WOO_DELIVERY_URL . 'public/css/flatpickr.min.css', array(), $this->version, 'all' );
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/dtod-woo-delivery-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( "flatpickr_js", dtod_WOO_DELIVERY_URL . 'public/js/flatpickr.min.js', [], $this->version, true );
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/dtod-woo-delivery-admin.js', array( 'jquery', 'animejs', 'selectWoo', 'flatpickr_js' ), $this->version, true );
	...
	public function dtod_woo_delivery_menus_sections() {

        add_menu_page(
			__('Prime Delivery', 'woo-delivery'),
            __('Prime Delivery', 'woo-delivery'),
			'manage_options',
			'dtod-woo-delivery-settings',
			array($this, "dtod_woo_delivery_main_layout"),
			"dashicons-cart",
			null
		);
	...
	public function dtod_woo_delivery_settings_link( $links ) {
    	if ( array_key_exists( 'deactivate', $links ) ) {
			$links['deactivate'] = str_replace( '<a', '<a class="dtod-woo-delivery-deactivate-link"', $links['deactivate'] );
		}
        $links[] = '<a href="admin.php?page=dtod-woo-delivery-settings">Settings</a>';
        return $links;
    }

    public function dtod_woo_delivery_process_delivery_timezone_form() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		parse_str( $_POST[ 'dateFormData' ], $date_form_data );
		$timezone_form_settings = [];

		$store_location_timezone = sanitize_text_field($date_form_data['dtod_delivery_time_timezone']);

		$timezone_form_settings['store_location_timezone'] = $store_location_timezone;


		if(get_option('dtod_woo_delivery_time_settings') == false) {
			update_option('dtod_woo_delivery_time_settings', $timezone_form_settings);
		} else {
			$timezone_form_settings = array_merge(get_option('dtod_woo_delivery_time_settings'),$timezone_form_settings);
			update_option('dtod_woo_delivery_time_settings', $timezone_form_settings);
		}
	...
	public function dtod_woo_delivery_process_delivery_option_settings() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		...
		$dtod_enable_option_time_pickup = !isset($form_data['dtod_enable_option_time_pickup']) ? false : true;

		$delivery_option_field_label = sanitize_text_field($form_data['dtod_woo_delivery_delivery_option_label']);

		$delivery_field_label = sanitize_text_field($form_data['dtod_woo_delivery_option_delivery_label']);

		$pickup_field_label = sanitize_text_field($form_data['dtod_woo_delivery_option_pickup_label']);

		$delivery_option_settings_form_settings['enable_option_time_pickup'] = $dtod_enable_option_time_pickup;
		...
		if(get_option('dtod_woo_delivery_option_delivery_settings') == false) {
			update_option('dtod_woo_delivery_option_delivery_settings', $delivery_option_settings_form_settings);
		} else {
			$delivery_option_settings_form_settings = array_merge(get_option('dtod_woo_delivery_option_delivery_settings'),$delivery_option_settings_form_settings);
			update_option('dtod_woo_delivery_option_delivery_settings', $delivery_option_settings_form_settings);
		}
		wp_send_json_success();
    ...
    public function dtod_woo_delivery_process_delivery_date_form() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		...
		$enable_delivery_date = !isset($date_form_data['dtod_enable_delivery_date']) ? false : true;
		
		$delivery_date_mandatory = !isset($date_form_data['dtod_delivery_date_mandatory']) ? false : true;
		
		$delivery_date_field_label = sanitize_text_field($date_form_data['dtod_delivery_date_field_label']);
		
		$delivery_date_selectable_date = sanitize_text_field($date_form_data['dtod_delivery_date_selectable_date']);
		
		$delivery_date_format = sanitize_text_field($date_form_data['dtod_delivery_date_format']);

		$auto_select_first_date = !isset($date_form_data['dtod_auto_select_first_date']) ? false : true;
		
		$delivery_week_starts_from = sanitize_text_field($date_form_data['dtod_delivery_date_week_starts_from']);
		
		$delivery_date_delivery_days="";
		if(isset($date_form_data['dtod_delivery_date_delivery_days'])) {
			$delivery_days = $this->helper->dtod_woo_delivery_array_sanitize($date_form_data['dtod_delivery_date_delivery_days']);
		...
		if(get_option('dtod_woo_delivery_date_settings') == false) {
			update_option('dtod_woo_delivery_date_settings', $date_form_settings);
		} else {
			$date_form_settings = array_merge(get_option('dtod_woo_delivery_date_settings'),$date_form_settings);
			update_option('dtod_woo_delivery_date_settings', $date_form_settings);
		}
		wp_send_json_success();
    ...
    public function dtod_woo_delivery_process_pickup_date_form() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		...
		$enable_pickup_date = !isset($date_form_data['dtod_enable_pickup_date']) ? false : true;
		
		$pickup_date_mandatory = !isset($date_form_data['dtod_pickup_date_mandatory']) ? false : true;

		$pickup_date_field_label = sanitize_text_field($date_form_data['dtod_pickup_date_field_label']);
		
		$pickup_date_selectable_date = sanitize_text_field($date_form_data['dtod_pickup_date_selectable_date']);
		
		$pickup_date_format = sanitize_text_field($date_form_data['dtod_pickup_date_format']);


		$pickup_week_starts_from = sanitize_text_field($date_form_data['dtod_pickup_date_week_starts_from']);
		
		$pickup_date_delivery_days="";
		if(isset($date_form_data['dtod_pickup_date_delivery_days'])) {
			$delivery_days = $this->helper->dtod_woo_delivery_array_sanitize($date_form_data['dtod_pickup_date_delivery_days']);
			$pickup_date_delivery_days = implode(',', $delivery_days);
		}

		$auto_select_first_pickup_date = !isset($date_form_data['dtod_auto_select_first_pickup_date']) ? false : true;
		...
		if(get_option('dtod_woo_delivery_pickup_date_settings') == false) {
			update_option('dtod_woo_delivery_pickup_date_settings', $date_form_settings);
		} else {
			$date_form_settings = array_merge(get_option('dtod_woo_delivery_pickup_date_settings'),$date_form_settings);
			update_option('dtod_woo_delivery_pickup_date_settings', $date_form_settings);
		}
    ...
    public function dtod_woo_delivery_process_delivery_date_offdays_form() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
    	$year_array = [];
    	$offdays_array = [];
    	parse_str( $_POST[ 'dateFormData' ], $date_form_data );
    	foreach($date_form_data as $key => $value) {
		    if (strpos($key, 'dtod_woo_delivery_offdays_year_') === 0) {
		        array_push($year_array,sanitize_text_field($value));
		    }
		}
		foreach($year_array as $year) {
			$offdays_months = $this->helper->dtod_woo_delivery_array_sanitize($date_form_data["dtod_woo_delivery_offdays_month_".$year]);
			if(!empty($offdays_months)){
				foreach($offdays_months as $offdays_month) {
					if($offdays_month != "") {
						$offdays_days = sanitize_text_field($date_form_data["dtod_woo_delivery_offdays_dates_".$offdays_month."_".$year]);
    ...
    public function dtod_woo_delivery_process_delivery_time_form() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		parse_str( $_POST[ 'dateFormData' ], $date_form_data );
		$time_form_settings = [];
		$enable_delivery_time = !isset($date_form_data['dtod_enable_delivery_time']) ? false : true;
		$delivery_time_mandatory = !isset($date_form_data['dtod_delivery_time_mandatory']) ? false : true;
		$delivery_time_field_label = sanitize_text_field($date_form_data['dtod_delivery_time_field_label']);
		$disable_current_time_slot = !isset($date_form_data['dtod_delivery_time_disable_current_time_slot']) ? false : true;
		$delivery_time_format = sanitize_text_field($date_form_data['dtod_delivery_time_format']);
		$delivery_time_maximum_order = sanitize_text_field($date_form_data['dtod_delivery_time_maximum_order']);
		$auto_select_first_time = !isset($date_form_data['dtod_auto_select_first_time']) ? false : true;
		$delivery_time_slot_starts_hour = (isset($date_form_data['dtod_delivery_time_slot_starts_hour']) && $date_form_data['dtod_delivery_time_slot_starts_hour'] !="") ? sanitize_text_field($date_form_data['dtod_delivery_time_slot_starts_hour']) : "0";
		
		$delivery_time_slot_starts_min = (isset($date_form_data['dtod_delivery_time_slot_starts_min']) && $date_form_data['dtod_delivery_time_slot_starts_min'] !="") ? sanitize_text_field($date_form_data['dtod_delivery_time_slot_starts_min']) : "0"; 

		$delivery_time_slot_starts_format = sanitize_text_field($date_form_data['dtod_delivery_time_slot_starts_format']);
		...
		$delivery_time_slot_ends_hour = (isset($date_form_data['dtod_delivery_time_slot_ends_hour']) && $date_form_data['dtod_delivery_time_slot_ends_hour'] !="") ? sanitize_text_field($date_form_data['dtod_delivery_time_slot_ends_hour']) : "0";
		
		$delivery_time_slot_ends_min = (isset($date_form_data['dtod_delivery_time_slot_ends_min']) && $date_form_data['dtod_delivery_time_slot_ends_min'] !="") ? sanitize_text_field($date_form_data['dtod_delivery_time_slot_ends_min']) : "0"; 

		$delivery_time_slot_ends_format = sanitize_text_field($date_form_data['dtod_delivery_time_slot_ends_format']);
		...
		$delivery_time_slot_duration_time = (isset($date_form_data['dtod_delivery_time_slot_duration_time']) && $date_form_data['dtod_delivery_time_slot_duration_time'] !="") ? sanitize_text_field($date_form_data['dtod_delivery_time_slot_duration_time']) : "0";
		$delivery_time_slot_duration_format = sanitize_text_field($date_form_data['dtod_delivery_time_slot_duration_format']);
		...
		if(get_option('dtod_woo_delivery_time_settings') == false) {
			update_option('dtod_woo_delivery_time_settings', $time_form_settings);
		} else {
			$time_form_settings = array_merge(get_option('dtod_woo_delivery_time_settings'),$time_form_settings);
			update_option('dtod_woo_delivery_time_settings', $time_form_settings);
		}
	...
	public function dtod_woo_delivery_process_pickup_time_form() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		parse_str( $_POST[ 'dateFormData' ], $pickup_form_data );
		$pickup_time_form_settings = [];
		$enable_pickup_time = !isset($pickup_form_data['dtod_enable_pickup_time']) ? false : true;
		$pickup_time_mandatory = !isset($pickup_form_data['dtod_pickup_time_mandatory']) ? false : true;
		$pickup_time_field_label = sanitize_text_field($pickup_form_data['dtod_pickup_time_field_label']);
		$disable_current_time_slot = !isset($pickup_form_data['dtod_pickup_time_disable_current_time_slot']) ? false : true;
		$pickup_time_format = sanitize_text_field($pickup_form_data['dtod_pickup_time_format']);
		$pickup_time_maximum_order = sanitize_text_field($pickup_form_data['dtod_pickup_time_maximum_order']);
		$auto_select_first_time = !isset($pickup_form_data['dtod_auto_select_first_pickup_time']) ? false : true;
		$pickup_time_slot_starts_hour = (isset($pickup_form_data['dtod_pickup_time_slot_starts_hour']) && $pickup_form_data['dtod_pickup_time_slot_starts_hour'] !="") ? sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_starts_hour']) : "0";
		
		$pickup_time_slot_starts_min = (isset($pickup_form_data['dtod_pickup_time_slot_starts_min']) && $pickup_form_data['dtod_pickup_time_slot_starts_min'] !="") ? sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_starts_min']) : "0"; 

		$pickup_time_slot_starts_format = sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_starts_format']);
		...
		$pickup_time_slot_ends_hour = (isset($pickup_form_data['dtod_pickup_time_slot_ends_hour']) && $pickup_form_data['dtod_pickup_time_slot_ends_hour'] !="") ? sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_ends_hour']) : "0";
		
		$pickup_time_slot_ends_min = (isset($pickup_form_data['dtod_pickup_time_slot_ends_min']) && $pickup_form_data['dtod_pickup_time_slot_ends_min'] !="") ? sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_ends_min']) : "0"; 

		$pickup_time_slot_ends_format = sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_ends_format']);
		...
		$pickup_time_slot_duration_time = (isset($pickup_form_data['dtod_pickup_time_slot_duration_time']) && $pickup_form_data['dtod_pickup_time_slot_duration_time'] !="") ? sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_duration_time']) : "0";
		$pickup_time_slot_duration_format = sanitize_text_field($pickup_form_data['dtod_pickup_time_slot_duration_format']);
		...
		if(get_option('dtod_woo_delivery_pickup_settings') == false) {
			update_option('dtod_woo_delivery_pickup_settings', $pickup_time_form_settings);
		} else {
			$pickup_time_form_settings = array_merge(get_option('dtod_woo_delivery_pickup_settings'),$pickup_time_form_settings);
			update_option('dtod_woo_delivery_pickup_settings', $pickup_time_form_settings);
		}
	...
	public function dtod_woo_delivery_process_localization_settings() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		...
		$order_limit_notice = sanitize_text_field($form_data['dtod_woo_delivery_order_limit_notice']);
		$pickup_limit_notice = sanitize_text_field($form_data['dtod_woo_delivery_pickup_limit_notice']);
		$delivery_details_text = sanitize_text_field($form_data['dtod_woo_delivery_delivery_details_text']);
		$order_metabox_heading = sanitize_text_field($form_data['dtod_woo_delivery_order_metabox_heading']);

		$checkout_date_notice = sanitize_text_field($form_data['dtod_woo_delivery_checkout_date_notice']);
		$checkout_time_notice = sanitize_text_field($form_data['dtod_woo_delivery_checkout_time_notice']);

		$checkout_pickup_date_notice = sanitize_text_field($form_data['dtod_woo_delivery_checkout_pickup_date_notice']);
		$checkout_pickup_time_notice = sanitize_text_field($form_data['dtod_woo_delivery_checkout_pickup_time_notice']);

		$checkout_delivery_option_notice = sanitize_text_field($form_data['dtod_woo_delivery_checkout_delivery_option_notice']);
		...
		if(get_option('dtod_woo_delivery_localization_settings') == false) {
			update_option('dtod_woo_delivery_localization_settings', $localization_settings_form_settings);
		} else {
			$localization_settings_form_settings = array_merge(get_option('dtod_woo_delivery_localization_settings'),$localization_settings_form_settings);
			update_option('dtod_woo_delivery_localization_settings', $localization_settings_form_settings);
		}
		wp_send_json_success();
    ...
    public function dtod_woo_delivery_process_other_settings() { 
    	check_ajax_referer('dtod_woo_delivery_nonce');
		...
		$field_position = sanitize_text_field($date_form_data['dtod_woo_delivery_field_position']);
		$other_settings_form_settings['field_position'] = $field_position;

		$dtod_disable_fields_for_downloadable_products = !isset($date_form_data['dtod_disable_fields_for_downloadable_products']) ? false : true;

		$other_settings_form_settings['disable_fields_for_downloadable_products'] = $dtod_disable_fields_for_downloadable_products;

		$custom_css = isset($date_form_data['dtod_woo_delivery_code_editor_css']) ? sanitize_textarea_field(htmlentities($date_form_data['dtod_woo_delivery_code_editor_css'])) : "";
		$other_settings_form_settings['custom_css'] = $custom_css;

		$delivery_heading_checkout = sanitize_text_field($date_form_data['dtod_woo_delivery_delivery_heading_checkout']);

		$other_settings_form_settings['delivery_heading_checkout'] = $delivery_heading_checkout;


		
		if(get_option('dtod_woo_delivery_other_settings') == false) {
			update_option('dtod_woo_delivery_other_settings', $other_settings_form_settings);
		} else {
			$other_settings_form_settings = array_merge(get_option('dtod_woo_delivery_other_settings'),$other_settings_form_settings);
			update_option('dtod_woo_delivery_other_settings', $other_settings_form_settings);
		}
		wp_send_json_success();
	...
	// CHECK FOR PRIME USER EMAIL EXIST
	...
	/* SETUP PRIME DISCOUNT */
	public function dtod_woo_setup_prime_settings () {
		check_ajax_referer('dtod_woo_delivery_nonce');
		
		$prime_form_settings = [];

		parse_str( $_POST[ 'dateFormData' ], $prime_form_data );

		$prime_set_default_delivery = sanitize_text_field($prime_form_data['dtod_woo_delivery_default_delivery']);
		$prime_form_settings['default_delivery'] = $prime_set_default_delivery;

		$prime_set_non_prime_amount = sanitize_text_field($prime_form_data['dtod_woo_delivery_non_prime_amount']);
		$prime_form_settings['non_prime_amount'] = $prime_set_non_prime_amount;

		$prime_set_default_prime_amount = sanitize_text_field($prime_form_data['dtod_woo_delivery_default_prime_amount']);
		$prime_form_settings['default_prime_amount'] = $prime_set_default_prime_amount;

		$prime_set_purchase_limit = sanitize_text_field($prime_form_data['dtod_woo_delivery_purchase_limit']);
		$prime_form_settings['purchase_limit'] = $prime_set_purchase_limit;

		$prime_set_product_id = sanitize_text_field($prime_form_data['dtod_woo_prime_product_id']);
		$prime_form_settings['prime_prod_id'] = $prime_set_product_id;

		// TRY PRIME
		$prime_try_set_product_id = sanitize_text_field($prime_form_data['dtod_woo_try_prime_product_id']);
		$prime_form_settings['prime_try_prod_id'] = $prime_try_set_product_id;

		if(get_option('dtod_woo_delivery_prime_settings') == false) {
			update_option('dtod_woo_delivery_prime_settings', $prime_form_settings);
		} else {
			$prime_form_settings = array_merge(get_option('dtod_woo_delivery_prime_settings'),$prime_form_settings);
			update_option('dtod_woo_delivery_prime_settings', $prime_form_settings);
		}
	...
	// CHANGE SHIPPING LABEL (CLALLA)
	public function dtod_custom_shipping_package_name( $name ) {
		return 'Delivery';
	}
	...
	*/
	public function dtod_woo_delivery_add_custom_fields_orders_list($columns) {
		
		$delivery_details_text = (isset(get_option('dtod_woo_delivery_localization_settings')['delivery_details_text']) && !empty(get_option('dtod_woo_delivery_localization_settings')['delivery_details_text'])) ? get_option('dtod_woo_delivery_localization_settings')['delivery_details_text'] : __("Delivery Details", "woo-delivery");
	...
	public function dtod_woo_delivery_show_custom_fields_data_orders_list($column) {
		...
		if($column == 'order_delivery_details')
		{
			...
			if(metadata_exists('post', $post->ID, 'delivery_time') && get_post_meta($post->ID, 'delivery_time', true) !="")
			{
				...
				// ADD SCHEDULE FEE TO ORDERS PAGE (CLALLA)	
				$user_email = $order->get_billing_email();;
				if ( ! d2d_prime_is_active_prime_member($user_email) ) {
					echo " <br > ";
					echo __('Schedule Fee', "woo-delivery").": " . wc_price($prime_set_default_prime_amount);
				} else {
					echo '<br/> <strong style="color:#2270b1">';
					echo __('Priority', "woo-delivery").": " . 'Prime';
					echo '</strong>';
				}
	...
	public function dtod_woo_delivery_show_custom_fields_data_orders_list_hpos($column, $order) {
	...
	public function dtod_woo_delivery_information_after_shipping_address($order){
	    ...
	    if((metadata_exists('post', $order_id, 'delivery_time') && get_post_meta($order_id,"delivery_time",true) != "") || ($order->meta_exists('delivery_time') && $order->get_meta( 'delivery_time', true ) != "")) {
			...
			$user_email = $order->get_billing_email();;
			if ( ! d2d_prime_is_active_prime_member($user_email) ) {
				echo '<p><strong>Schedule Fee: </strong> '.wc_price($prime_set_default_prime_amount).'</p>';
			} else {
				echo '<p><strong>Priority: </strong> Prime</p>';
			}	    			
	    }
	...
	public function dtod_woo_delivery_review_notice() {
	    $options = get_option('dtod_woo_delivery_review_notice');

	    $activation_time = get_option('dtod-woo-delivery-activation-time');

	    $notice = '<div class="dtod-woo-delivery-review-notice notice notice-success is-dismissible">';
        $notice .= '<img class="dtod-woo-delivery-review-notice-left" src="'.dtod_WOO_DELIVERY_URL.'admin/images/woo-delivery-logo.png" alt="dtod-woo-delivery">';
        $notice .= '<div class="dtod-woo-delivery-review-notice-right">';
	...
	public function dtod_woo_delivery_save_review_notice() {
	    $notice = sanitize_text_field($_POST['notice']);
	    $value = array();
	    $value['review_notice'] = $notice;
	    $value['updated_at'] = time();

	    update_option('dtod_woo_delivery_review_notice',$value);
	    wp_send_json_success($value);
	}


	public function dtod_woo_delivery_custom_meta_box() {
		$order_metabox_heading = (isset(get_option('dtod_woo_delivery_localization_settings')['order_metabox_heading']) && !empty(get_option('dtod_woo_delivery_localization_settings')['order_metabox_heading'])) ? get_option('dtod_woo_delivery_localization_settings')['order_metabox_heading'] : __("Delivery/Pickup Date & Time", "woo-delivery");
		
		$screen = wc_get_container()->get( CustomOrdersTableController::class )->custom_orders_table_usage_is_enabled() ? wc_get_page_screen_id( 'shop-order' ) : 'shop_order';
		add_meta_box( 'dtod_woo_delivery_meta_box', $order_metabox_heading, array($this,"dtod_woo_delivery_meta_box_markup"), $screen, 'normal', 'high', null );
	}

	public function dtod_woo_delivery_meta_box_markup($post_or_order_object) {
		// if any timezone data is saved, set default timezone with the data
		$timezone = $this->helper->get_the_timezone();
		date_default_timezone_set($timezone);
	    ...
	    $time_options = dtod_Woo_Delivery_Time_Option::delivery_time_option($delivery_time_settings,"meta_box");
	    $pickup_options = dtod_Woo_Delivery_Pickup_Option::pickup_time_option($pickup_time_settings,"meta_box");

	    $delivery_options = dtod_Woo_Delivery_Delivery_Option::delivery_option($delivery_option_settings,"meta_box");
        ...
        $meta_box = '<div data-today_date="'.$today.'" id="dtod_woo_delivery_admin_setting_wrapper">';
        $meta_box .= '<input type="hidden" id="dtod_woo_delivery_meta_box_order_id" value="' . $order_id . '" readonly>';
        if(!$enable_delivery_option) {
        	$meta_box .= '<div style="display:none">';
        }
	    $meta_box .= '<select style="width:100%;margin:5px auto;" name ="dtod_woo_delivery_meta_box_delivery_selection_field" id="dtod_woo_delivery_meta_box_delivery_selection_field">';
	    		if($delivery_type == '') {
        ...
        if($enable_delivery_date) {
	        ...
	        $meta_box .= '<input style="width:100%;margin:5px auto;display:none" type="text" id="dtod_woo_delivery_meta_box_datepicker" placeholder="'.$delivery_date_field_label.'" name="dtod_woo_delivery_meta_box_datepicker" data-disable_dates="'.$disable_dates.'" data-selectable_dates="'.$selectable_date.'" data-disable_week_days="'.$disable_week_days.'" data-week_starts_from="'.$week_starts_from.'" data-date_format="'.$delivery_date_format.'" value="' . $delivery_date . '">';
    	}
    	...
    	if($enable_pickup_date) {
	        ...
	        $meta_box .= '<input style="width:100%;margin:5px auto;display:none" type="text" id="dtod_woo_delivery_meta_box_pickup_datepicker" placeholder="'.$pickup_date_field_label.'" name="dtod_woo_delivery_meta_box_pickup_datepicker" data-pickup_disable_dates="'.$pickup_disable_dates.'" data-pickup_selectable_dates="'.$pickup_selectable_date.'" data-pickup_disable_week_days="'.$pickup_disable_week_days.'" data-pickup_week_starts_from="'.$pickup_week_starts_from.'" data-pickup_date_format="'.$pickup_date_format.'" value="' . $pickup_date . '">';
    	}


    	if($enable_delivery_time) {
    		$meta_box .= '<select style="width:100%;margin:5px auto;display:none" name ="dtod_woo_delivery_meta_box_time_field" id="dtod_woo_delivery_meta_box_time_field" data-order_limit_notice="'.$order_limit_notice.'">';
    	...
    	if($enable_pickup_time) {
    		$meta_box .= '<select style="width:100%;margin:5px auto;display:none" name ="dtod_woo_delivery_meta_box_pickup_field" id="dtod_woo_delivery_meta_box_pickup_field" data-pickup_limit_notice="'.$pickup_limit_notice.'">';
    	...
    	$meta_box .= '<div class="dtod-woo-delivery-metabox-update-section" data-plugin-url="'.dtod_WOO_DELIVERY_URL.'">';
        $meta_box .= '<a class="dtod-woo-delivery-metabox-update-btn" href="#" style="margin-right:10px"><button type="button" class="button button-primary">'.__("Update", "woo-delivery").'</button></a>';
	...
	public function dtod_woo_delivery_save_meta_box_information() {
		check_ajax_referer('dtod_woo_delivery_nonce');
		$delivery_time_settings = get_option('dtod_woo_delivery_time_settings');
		// if any timezone data is saved, set default timezone with the data
		$timezone = $this->helper->get_the_timezone();
	...
	public function dtod_woo_delivery_meta_box_get_orders() {
	...
	public function dtod_woo_delivery_meta_box_get_orders_pickup() {
	...
	public function dtod_woo_delivery_admin_disable_max_delivery_pickup_date() {
    ...
    public function dtod_woo_delivery_main_layout() {
        include_once dtod_WOO_DELIVERY_DIR . '/admin/partials/dtod-woo-delivery-admin-display.php';
    }


    public function dtod_woo_delivery_add_delivery_type() {
    	delete_option('dtod_woo_delivery_date_time_change');
    }
